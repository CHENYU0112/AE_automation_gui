...
def _isint(string):
    ...
    False
    """
    return type(string) is int or (isinstance(string, (_binary_type, _text_type))) and _isconvertible(int, string)
...
def _align_column(strings, alignment, minwidth=0, has_invisible=True):
    ...
    maxwidth = max(*list(map(width_fn, strings)), minwidth)
    ...
    * pandas.DataFrame (usually used with headers="keys")
    ...
    if hasattr(tabular_data, "keys") and hasattr(tabular_data, "values"):
        ...
        if callable(tabular_data.values):
            ...
            msg = "tabular data doesn't appear to be a dict or a DataFrame"
        ...
        if headers == "keys" and hasattr(tabular_data, "dtype") and tabular_data.dtype.names:
            # numpy record array
            ...
            if headers == "keys":
                ...
                msg = "headers for a list of dicts is not a dict or a keyword"
                raise ValueError(msg)
...
def tabulate(
    ...
    if headers:
        ...
        headers = [_align_header(h, a, minw) for h, a, minw in zip(headers, t_aligns, minwidths)]
        rows = list(zip(*cols))
    else:
...
def _main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "h1o:s:f:", ["help", "header", "output", "sep=", "format="])
