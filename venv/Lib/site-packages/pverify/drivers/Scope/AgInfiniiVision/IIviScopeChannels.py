...
from .IIviScopeChannel import IIviScopeChannel


class IIviScopeChannels:
    def __init__(self, root):
        self.root = root
        """:type : pverify.drivers.Scope.AgInfiniiVision.IIviScope.IIviScope"""
        self.__channels = None
        """:type : dict[str,IIviScopeChannel]"""
    ...
    def Item(self, OutputName):
        ...
        :return: A pointer to a class-compliant IIviScopeChannel interface.
        ...
        if self.__channels is None:
            ...
            for i in range(self.Count):
                name = self.Name(i + 1)
                self.__channels.update({name: IIviScopeChannel(self.root, name)})
