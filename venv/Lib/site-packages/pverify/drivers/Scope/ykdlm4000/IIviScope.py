...
class IIviScope(IVICOMDriverWrapper, IIviScopeBase.IIviScopeBase):
    ...
    def Initialize(self, ResourceName, IdQuery=1, Reset=1, OptionString="Calibrate=True"):
        ...
        if True:
            ...
            ret = ret.strip()
            self.IDN = ret
            if ret.find("YOKOGAWA,DLM4") == -1:
                self.__Initialized = False
                self.interface.vi_close()
                msg = "ykdlm4000: Cant't find '{}' in the returned IDN string '{}'".format(ret, "YOKOGAWA,DLM4")
                raise ValueError(msg)
            try:
                self.GetError()
            except InstrumentError as ie:
                if self._log is not None:
                    # Sometimes the scope hangs and has to be turned off manually. When restarting and doing the first
                    # error query there will be a "Query Interrupted Error". If not cleared here it will falsly pop out
                    # on the first function doing an error query internally
                    self._log.debug(
                        f"Suppressed errors:\n\tDescription: {ie}, Status: {ie.errorID}",
                        exc_info=False,
                    )
        if Reset:
            self.interface.vi_write(":CHUT:ALL:DISP OFF")
            self.Utility.Reset()
            self.GetError()
        ...
        if OptionString.upper().find("CALIBRATE=TRUE") != -1:
            self.interface.set_visa_attribute(VI_ATTR_TMO_VALUE, 7000)
...
if __name__ == "__main__":
    scope = IIviScope()
    scope.Initialize("foo", 1, 1, "")
    scope.ch1 = scope.Channels.Item("C1")
    print("foo")
