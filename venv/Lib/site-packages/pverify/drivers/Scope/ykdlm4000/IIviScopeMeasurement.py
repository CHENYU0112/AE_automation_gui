...
class IIviScopeMeasurement(IIviScopeBase.IIviScopeBase):
    ...
    def FetchWaveformMeasurement(self, MeasFunction):
        ...
        if (
            self.interface.vi_query(
                f":MEAS:CHAN{self._Channel}:{MeasFunction.value}:STAT?",
                rformat="%d",
            )
            == 0
        ):
            self.interface.vi_write(f":MEAS:CHAN{self._Channel}:{MeasFunction.value}:STAT ON;")
            self.interface.vi_write(":MEAS:MODE ON")

        try:
            value = self.interface.vi_query(f":MEAS:CHAN{self._Channel}:{MeasFunction.value}:VAL?", rformat="%f")
        except Exception:
            try:
                sleep(0.5)
                value = self.interface.vi_query(
                    f":MEAS:CHAN{self._Channel}:{MeasFunction.value}:VAL?",
                    rformat="%f",
                )
            except Exception:
                sleep(0.5)
    ...
    def ReadWaveform(self, MaxTimeMilliseconds=5000):
        ...
        :return: Returns a waveform datatype as dictionary of form {"time_scope": <np.ndarray>, "data_scope": <np.ndarray>}
        """
        from pverify.drivers.Scope.ykdlm4000.IIviScopeAcquisition import IIviScopeAcquisition
        from pverify.drivers.Scope.ykdlm4000.IIviScopeMeasurements import IIviScopeMeasurements as SM
        ...
        if (ElapsedTime > MaxTimeMilliseconds) and AcqInProg:
            msg = "ykdlm4000: A timeout occurred while acquisition was in progress"
            raise Exception(msg)
        elif not AcqInProg:
    ...
    def AutoRangeMath(self):
        ...
        if self._Channel in ["MATH1", "MATH2", "MATH3", "MATH4"]:
            self.interface.vi_write(f":{self._Channel}:USER:SCAL:ARAN")
            sleep(1)
