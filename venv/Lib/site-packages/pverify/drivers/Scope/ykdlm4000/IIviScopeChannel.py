...
class IIviScopeChannel(IIviScopeBase.IIviScopeBase):
    ...
    Methods:
        ...
        *AutoSetup()
            Performs an automatic setup of the channel (Range, Offset).

        *Label(Labelname, Enabled)
            The waveform name that is shown on the scope's screen

        *DZCalibrate
            Performs demagnetization and zero calibration of the probe.
    ...
    def AutoSetup(self):
        ...
        if fwversion >= 2.47:
            self.interface.vi_write(f":CHAN{self._Channel}:ASC:EXEC")
            self.GetError()
        else:
            msg = f"The scope's firmware (F{fwversion:.2f}) has to be at least version F2.47"
            raise Exception(msg)
    ...
    def DZCalibrate(self):
        """
        NON-COMPLIANCE FUNCTION
        Performs demagnetization and zero calibration of the probe.
        """
        self.interface.vi_write(f":CHAN{self._Channel}:PROB:DZC")
        sleep(3.0)
        self.GetError()
    ...
    def PZCalibrate(self):
        ...
        Performs zero calibration of the current probe.
        """
        self.interface.vi_write(f":CHAN{self._Channel}:PROB:PZC")
        sleep(3.0)
        self.GetError()
