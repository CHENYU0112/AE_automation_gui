...
from pverify.drivers.PowerAnalyzer.ykwt500.IIfxPwrAnWaveform import IIfxPwrAnWaveform
...
class IIfxPwrAnWaveforms(IIfxPwrAnBase.IIfxPwrAnBase):
    ...
    def Item(self, Source):
        ...
        """
        if Source in ["U1", "U2", "U3", "I1", "I2", "I3"]:
            return IIfxPwrAnWaveform(self, Source)
        else:
            msg = f"Channel '{Source}' does not exist. This power analyzer only has channel 1-3."
    ...
    def Name(self, Index):
        ...
        """
        if (int(Index) >= 1) and (int(Index) <= 3):
            return ["U1", "U2", "U3", "I1", "I2", "I3"][int(Index) - 1]
        else:
            msg = "ykwt3000: Channel index exceeds the limits 1-3"
            raise IndexError(msg)
