...
from pverify.drivers.PowerAnalyzer.ykwt1800.IIfxPwrAnMeasurement import IIfxPwrAnMeasurement
...
class IIfxPwrAnMeasurements(IIfxPwrAnBase.IIfxPwrAnBase):
    ...
    def Item(self, Element, PwrAnMeasItem):
        ...
        """
        if (int(Element) >= 1) and (int(Element) <= 6):
            ret = IIfxPwrAnMeasurement(self, Element, PwrAnMeasItem, self.__ItemCount, False)
            if self.__ItemCount < 255:
                self.__ItemCount += 1
            else:
                self.__ItemCount = 1
            self.GetError()
            ...
            msg = f"Channel '{Element}' does not exist. This power analyzer only has channel 1-6."
    ...
    def Item_Harmonic(self, Element, PwrAnMeasItem):
        ...
        """
        if (int(Element) >= 1) and (int(Element) <= 6):
            ret = IIfxPwrAnMeasurement(self, Element, PwrAnMeasItem, self.__ItemCountHarm, True)
            if self.__ItemCountHarm < 64:
                self.__ItemCountHarm += 1
            else:
                self.__ItemCountHarm = 1
            self.GetError()
