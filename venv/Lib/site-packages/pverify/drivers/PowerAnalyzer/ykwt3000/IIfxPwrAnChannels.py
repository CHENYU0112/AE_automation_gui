...
from pverify.drivers.PowerAnalyzer.ykwt3000.IIfxPwrAnChannel import IIfxPwrAnChannel
...
class IIfxPwrAnChannels(IIfxPwrAnBase.IIfxPwrAnBase):
    ...
    def Item(self, Name):
        ...
        :param Name: The channel number (1-4)
        :type Name: int
        :rtype: IIfxPwrAnChannel
        """
        if str(Name) in ["1", "2", "3", "4"]:
            return self.__channels[str(Name)]
        else:
            msg = 'ykwt3000: Channel name has to be one of ["1", "2", "3", "4"]'
    ...
    def Name(self, Index):
        ...
        """
        if (int(Index) > 0) and (int(Index) <= 4):
            return ["1", "2", "3", "4"][int(Index) - 1]
        else:
