...
class IoManager:
    ...
    def get_gdb_response(self, timeout_sec: float = DEFAULT_GDB_TIMEOUT_SEC, raise_error_on_timeout=True):
        ...
        retval = self._get_responses_windows(timeout_sec) if USING_WINDOWS else self._get_responses_unix(timeout_sec)

        if not retval and raise_error_on_timeout:
            msg = f"Did not get response from gdb after {timeout_sec} seconds"
            raise GdbTimeoutError(msg)
    ...
    def _get_responses_windows(self, timeout_sec):
        ...
        while True:
            ...
            if timeout_sec == 0:
                break
            if responses_list and self._allow_overwrite_timeout_times:
    ...
    def _get_responses_list(self, raw_output: bytes, stream: str) -> list[dict[Any, Any]]:
        ...
        ) = _buffer_incomplete_responses(raw_output, self._incomplete_output.get(stream))
...
def _buffer_incomplete_responses(raw_output: bytes | None, buf: bytes | None) -> tuple[bytes | None, bytes | None]:
