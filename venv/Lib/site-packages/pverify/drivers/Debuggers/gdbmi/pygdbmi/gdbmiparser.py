...
def _setup_logger(logger, debug):
    ...
    handler.setFormatter(logging.Formatter("[%(filename)s:%(lineno)s - %(funcName)20s() ] %(message)s"))
...
def parse_response(gdb_mi_text: str) -> dict:
    ...
    if _GDB_MI_NOTIFY_RE.match(gdb_mi_text):
        ...
        payload = match.groups()[0] if match else None
        return {
            "type": "console",
            "message": None,
            "payload": payload,
...
def response_is_finished(gdb_mi_text: str) -> bool:
    ...
    """
    return bool(_GDB_MI_RESPONSE_FINISHED_RE.match(gdb_mi_text))
