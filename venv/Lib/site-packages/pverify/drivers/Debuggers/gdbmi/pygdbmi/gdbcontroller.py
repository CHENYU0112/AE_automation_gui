...
from .constants import DEFAULT_GDB_TIMEOUT_SEC, DEFAULT_TIME_TO_CHECK_FOR_ADDITIONAL_OUTPUT_SEC
...
class GdbController:
    def __init__(
        ...
        gdb_path = command[0]
        if not gdb_path:
            msg = "a valid path to gdb must be specified"
            raise ValueError(msg)

        else:
            abs_gdb_path = find_executable(gdb_path)
            if abs_gdb_path is None:
                msg = f'gdb executable could not be resolved from "{gdb_path}"'
                raise ValueError(msg)
    ...
    def write(
        ...
        return self.io_manager.write(mi_cmd_to_write, timeout_sec, raise_error_on_timeout, read_response)
