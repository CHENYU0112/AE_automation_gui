# Vendored from https://github.com/cfobel/si-prefix to support Python 3.12
...
CRE_SI_NUMBER = re.compile(
    r"\s*(?P<sign>[\+\-])?" r"(?P<integer>\d+)" r"(?P<fraction>.\d+)?\s*" f"(?P<si_unit>[{SI_PREFIX_UNITS}])?\s*"
)
...
def si_format(value, precision=1, format_str="{value} {prefix}", exp_format_str="{value}e{expof10}"):
    """
...
def si_parse(value):
    ...
    CRE_10E_NUMBER = re.compile(
        r"^\s*(?P<integer>[\+\-]?\d+)?" r"(?P<fraction>.\d+)?\s*([eE]\s*" r"(?P<expof10>[\+\-]?\d+))?$"
    )
    CRE_SI_NUMBER = re.compile(
        r"^\s*(?P<number>(?P<integer>[\+\-]?\d+)?" r"(?P<fraction>.\d+)?)\s*" f"(?P<si_unit>[{SI_PREFIX_UNITS}])?\s*$"
    )
